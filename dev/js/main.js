/**
 * Map Zoom Functionallity
 * TODO zoom shit mobile
 */
(function() {
...
/**
 * titleCase function that capitalizes strings
 */
...
function startCountdown() {
    ...
    countDownInterval = setInterval(function() {
        ...
        if (typeof(document.getElementById('minutes')) != 'undefined' && document.getElementById('minutes') != null) {
 ...
 */
function changeLivePanelStats(countryName,lastStats){

    /**
     * Remove Live update
     */
    if (lastStats.liveCases > 0 || lastStats.liveRecovered > 0 || lastStats.liveDeaths > 0){

        if (didRemoveBlinks === false){
            $('path').removeClass('countryCanimationSixty countryRanimationSixty countryDanimationSixty');
            didRemoveBlinks = true;
        }
    }

    /**
     * Some Country Mapping
     */
    if (countryName === 'US'){countryName = 'United States';}

    if (lastStats.liveCases > 0){
        $('path[title="'+countryName+'"]').addClass('countryCanimationSixty');
    }
    if (lastStats.liveRecovered > 0){
        $('path[title="'+countryName+'"]').addClass('countryRanimationSixty');
    }
    if (lastStats.liveDeaths > 0){
        $('path[title="'+countryName+'"]').addClass('countryDanimationSixty');
    }
 ...
 * This function updates the panel data on demand
 */
function updateLivePanelData(countryName,lastStats){

    $('.info-element').removeClass('counterAnimationTen counterAnimationSixty');

    $('.timeFetched').text(lastStatisticsTime);
    $('.countryN').text(countryName);
    $('.totalC').text(lastStats.confirmed);
    $('.cases-note').text('+'+lastStats.liveCases);
    $('.totalA').text(lastStats.active_cases);
    $('.totalR').text(lastStats.recovered);
    $('.recovered-note').text('+'+lastStats.liveRecovered);
    $('.totalD').text(lastStats.deaths);
    $('.death-note').text('+'+lastStats.liveDeaths);
    $('.newC').text(lastStats.new_cases);
    $('.newD').text(lastStats.new_deaths);
    $('.perM').text(lastStats.total_cases_per_1m_population);

    if (lastStats.liveCases > 0){
        $('.cases-note').closest('.info-element').addClass('counterAnimationSixty');
    }
    if (lastStats.liveRecovered > 0){
        $('.recovered-note').closest('.info-element').addClass('counterAnimationSixty');
    }
    if (lastStats.liveDeaths > 0){
    ...
    $.each(countriesHistoryData,function (countryName,countryDaysData) {
        ...
        $.each(currentDataLive,function () {
            ...
            if (
                 ...
                 */
                var liveCountryCases = parseInt(countryData.eq(2).text().replace(',','')) - countryDaysData[countryDaysData.length-1].confirmed,
                    liveCountryDeaths = parseInt(countryData.eq(3).text().replace(',','')) - countryDaysData[countryDaysData.length-1].deaths,
                    liveCountryRecovered =parseInt(countryData.eq(4).text().replace(',','')) - countryDaysData[countryDaysData.length-1].recovered;
...
function doTheInit() {
    ...
    $(document).ready(function() {
        ...
        body.on('click','.toggle-sources',function () {
            ...
            body.find('.sourcesIndicator').toggleClass('toggled');
        });
        ...
        svgMap.on('mouseenter','path',function () {
            ...
            if (undefined !== countryDaysData){
                ...
                updateLivePanelData(countryName,countryDaysData[countryDaysData.length-1]);
            }else{
                updateLivePanelData('World',countriesAllCurrentData['World']);
            }
        });
